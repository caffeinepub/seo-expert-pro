import { r as reactExports, j as jsxRuntimeExports, L as Link } from "./index-DSwezMv8.js";
import { W as WebGLRenderer, A as ACESFilmicToneMapping, S as Scene, P as PerspectiveCamera, b as AmbientLight, c as PointLight, I as IcosahedronGeometry, M as MeshBasicMaterial, d as Mesh, T as TorusKnotGeometry, e as OctahedronGeometry, f as BufferGeometry, g as BufferAttribute, h as PointsMaterial, i as Points, j as SphereGeometry, k as MeshStandardMaterial, l as DodecahedronGeometry, m as TorusGeometry } from "./three.module-CDwbhIs2.js";
...
const samplePosts = [
  {
    title: "Ultimate SEO Roadmap for Beginners in 2026: Step-by-Step Strategy to Grow Your Website",
    excerpt: "A complete beginner's guide to SEO in 2026 — covering keyword research, on-page optimization, technical SEO, link building, and content strategy in one step-by-step roadmap.",
    tags: ["Best Practices", "Keyword Research"],
    readTime: 18,
    slug: "ultimate-seo-roadmap-beginners-2026",
    image: "/assets/uploads/seo-roadmap-for-beginners-in-2026-019d1eeb-c6fe-7158-a287-def1dce147bb-1.webp"
  },
  {
...
function HeroBlogCanvas() {
  ...
  reactExports.useEffect(() => {
    ...
    camera.position.z = 7;
    const ambientLight = new AmbientLight(3721610, 0.4);
    scene.add(ambientLight);
    const pointLight1 = new PointLight(3721610, 2.5, 20);
    pointLight1.position.set(-4, 3, 3);
    scene.add(pointLight1);
    const pointLight2 = new PointLight(54527, 2, 18);
    pointLight2.position.set(5, -2, 4);
    scene.add(pointLight2);
    const pointLight3 = new PointLight(5213943, 1.5, 15);
    pointLight3.position.set(0, -4, 2);
    scene.add(pointLight3);
    const allGeos = [];
    const allMats = [];
    const bgIcoGeo = new IcosahedronGeometry(2.5, 1);
    const bgIcoMat = new MeshBasicMaterial({
      color: 3721610,
      wireframe: true,
      transparent: true,
      opacity: 0.05
    });
    const bgIco = new Mesh(bgIcoGeo, bgIcoMat);
    bgIco.position.set(0, 0, -3);
    scene.add(bgIco);
    allGeos.push(bgIcoGeo);
    allMats.push(bgIcoMat);
    const topIcoGeo = new IcosahedronGeometry(0.7, 0);
    ...
    for (const d of topIcoData) {
      ...
      allMats.push(mat);
      const m = new Mesh(topIcoGeo, mat);
    ...
    const botTkGeo = new TorusKnotGeometry(0.45, 0.13, 64, 8, 2, 3);
    allGeos.push(botTkGeo);
    const botKnotData = [
      { x: -4, y: -3, scale: 0.9, color: 54527, phase: 0 },
      { x: 0.5, y: -3.2, scale: 1.05, color: 3721610, phase: 1.6 },
      { x: 4.5, y: -2.8, scale: 0.85, color: 5213943, phase: 3 }
    ];
    const botKnots = [];
    for (const d of botKnotData) {
      ...
      allMats.push(mat);
      const m = new Mesh(botTkGeo, mat);
      m.position.set(d.x, d.y, -1.5);
      m.scale.setScalar(d.scale);
      scene.add(m);
      botKnots.push({ mesh: m, phase: d.phase, baseY: d.y });
    }
    const octGeo = new OctahedronGeometry(0.4, 0);
    allGeos.push(octGeo);
    const leftOctData = [
      { x: -6.2, y: 2, scale: 0.75, phase: 0 },
      { x: -5.8, y: 0.2, scale: 0.6, phase: 1 },
    ...
    const leftPartGeo = new BufferGeometry();
    leftPartGeo.setAttribute(
      "position",
      new BufferAttribute(leftPartPos, 3)
    );
    allGeos.push(leftPartGeo);
    const leftPartMat = new PointsMaterial({
      color: 3721610,
      size: 0.06,
      transparent: true,
      opacity: 0.65
    ...
    const leftParticles = new Points(leftPartGeo, leftPartMat);
    scene.add(leftParticles);
    const rightSphGeo = new SphereGeometry(0.35, 12, 8);
    allGeos.push(rightSphGeo);
    const rightSphData = [
      { x: 6, y: 2.2, scale: 0.85, phase: 0.5, filled: false },
    ...
    for (const d of rightDodecData) {
      ...
      allMats.push(mat);
      const m = new Mesh(dodecGeo, mat);
      m.position.set(d.x, d.y, -0.6);
      m.scale.setScalar(d.scale);
      scene.add(m);
      rightDodecs.push({ mesh: m, phase: d.phase, baseY: d.y });
    }
    const richSphGeo = new SphereGeometry(0.5, 32, 16);
    ...
    for (const d of richSphData) {
      ...
      allMats.push(mat);
      const m = new Mesh(richSphGeo, mat);
    ...
    const bgPartGeo = new BufferGeometry();
    bgPartGeo.setAttribute("position", new BufferAttribute(bgPosArr, 3));
    allGeos.push(bgPartGeo);
    const bgPartMat = new PointsMaterial({
      color: 3721610,
      size: 0.032,
      transparent: true,
      opacity: 0.35
    ...
    const bgParticles = new Points(bgPartGeo, bgPartMat);
    ...
    const animate = () => {
      ...
      frame += 5e-3;
      bgIco.rotation.x += 8e-4;
      bgIco.rotation.y += 12e-4;
      ...
      for (let i = 0; i < botKnots.length; i++) {
        ...
        mesh.rotation.x += 4e-3 + i * 1e-3;
        mesh.rotation.y += 3e-3 + i * 15e-4;
        mesh.rotation.z += 2e-3;
        mesh.position.y = baseY + Math.sin(frame * 0.7 + phase) * 0.18;
      }
      for (let i = 0; i < leftOcts.length; i++) {
        const { mesh, phase, baseY } = leftOcts[i];
        mesh.rotation.x += 5e-3 + i * 1e-3;
        mesh.rotation.z += 4e-3 + i * 12e-4;
        mesh.position.y = baseY + Math.sin(frame * 1.1 + phase) * 0.12;
      }
      const lPos = leftPartGeo.attributes.position;
      for (let i = 0; i < leftPartCount; i++) {
        lPos.array[i * 3 + 1] = lPos.array[i * 3 + 1] + 8e-3;
        if (lPos.array[i * 3 + 1] > 4) lPos.array[i * 3 + 1] = -4;
      }
      lPos.needsUpdate = true;
      for (let i = 0; i < rightSphs.length; i++) {
        const { mesh, phase, baseY } = rightSphs[i];
        mesh.rotation.x += 3e-3 + i * 2e-3;
        mesh.rotation.y += 5e-3 + i * 1e-3;
      ...
      bgParticles.rotation.y += 4e-4;
      bgParticles.rotation.x += 2e-4;
...
function Blog() {
  ...
  const [subscribeSuccess, setSubscribeSuccess] = reactExports.useState(false);
  const [subscribeError, setSubscribeError] = reactExports.useState("");
  reactExports.useEffect(() => {
    const mapped = samplePosts.map((p, index) => ({
      id: BigInt(index),
      title: p.title,
      excerpt: p.excerpt,
      tags: p.tags,
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              style: {
                ...
                width: "400px",
                height: "400px",
                background: "radial-gradient(circle, rgba(56,201,138,0.12) 0%, transparent 70%)",
                pointerEvents: "none"
              }
            }
          ),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              style: {
                ...
                width: "700px",
                height: "350px",
                background: "radial-gradient(ellipse, rgba(56,201,138,0.06) 0%, transparent 70%)",
                pointerEvents: "none"
              }
            }
          ),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                ...
                subscribeError && /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                ...
                subscribeSuccess && /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                    style: {
                      color: "#38C98A",
                    ...
                    children: "You're subscribed! Thanks for joining the RankPro community."
                  }
                )
              ]
            }
