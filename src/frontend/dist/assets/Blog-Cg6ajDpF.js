import { r as reactExports, j as jsxRuntimeExports, L as Link } from "./index-D0kYHSzF.js";
  ...
  reactExports.useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < icos.length; i++) {
        icos[i].rotation.x += 3e-3;
        icos[i].rotation.y += 4e-3;
        icos[i].position.y += Math.sin(frame + i) * 2e-3;
      }
      for (let i = 0; i < tori.length; i++) {
        tori[i].rotation.x += 2e-3;
        tori[i].rotation.z += 3e-3;
      }
      particles.rotation.y += 6e-4;
